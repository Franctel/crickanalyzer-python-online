...
function emailDetailShow() {
      ...
      Array.from(document.querySelectorAll("#mail-list tr.unread")).forEach(
        function (t) {
          t.classList.contains("unread") &&
            e.target.closest("tr").classList.remove("unread");
        }
      );
    });
  });
  var e = document.getElementById("emailOverview");
  Array.from(document.querySelectorAll("#closeChatRightSidebar")).forEach(
    ...
    Array.from(document.querySelectorAll("td.mailBox")).forEach(function (t) {
      t.addEventListener("click", function (t) {
        e.classList.remove("hidden"),
          document.querySelector("#emailList").classList.add("hidden");
      });
    }),
    document.querySelectorAll("#mail-list td.mailbox").forEach(function (e) {
...
function checkBoxAll() {
  document.querySelector("#checkboxAll").addEventListener("change", (e) => {
    Array.from(document.querySelectorAll(".itemCheckbox")).forEach(function (
      ...
      1 == e.target.checked ? (t.checked = !0) : (t.checked = !1);
    }),
    ...
    Array.from(document.querySelectorAll("#mail-list input")).forEach(function (
      ...
      e.addEventListener("click", function (e) {
        1 == e.target.checked
          ...
          : e.target.closest("tr").classList.remove("checked");
      });
    });
  ...
  function t() {
    ...
    Array.from(e).forEach(function (e) {
      ...
      (this.onclick = r),
      removeItems(),
      document.querySelectorAll("#mail-list tr").forEach((e) => {
  ...
  Array.from(e).forEach(function (e) {
    e.addEventListener("click", function (e) {
      var t = document.querySelectorAll(
        ...
        r = document.getElementById("checkboxAll"),
        a = document.querySelectorAll(
          ".mail-primary .checkbox-wrapper-mail input:checked"
        ).length;
      (r.checked = a > 0),
        (r.indeterminate = a > 0 && a < t.length),
        e.target.closest("li").classList.contains("active"),
        (document.getElementById("email-topbar-actions").style.display =
          a > 0 ? "block" : "none");
    });
  }),
    (document.getElementById("checkboxAll").onclick = t);
}
getJSON("mail-list.init.json", function (e, t) {
  null !== e
    ? console.log("Something went wrong: " + e)
    : loadMailData((allmaillist = t));
});
var markAllReadBtn = document.getElementById("mark-all-read");
markAllReadBtn.addEventListener("click", function (e) {
  0 === document.querySelectorAll("#mail-list tr.unread").length &&
          ...
          Array.from(document.querySelectorAll("#mail-list tr")).forEach(
            function (e) {
              ...
              if (e.classList.contains("active")) {
                ...
                (t = a),
                  (r = allmaillist.filter(function (e) {
                    return e.id != t;
                  })),
                  (allmaillist = r),
                  e.remove();
...
function removeSingleItem() {
  ...
  document.querySelectorAll(".remove-mail").forEach(function (t) {
    t.addEventListener("click", function (r) {
      (e = t.getAttribute("data-remove-id")),
        document
          .getElementById("delete-record")
          .addEventListener("click", function () {
            var t, r;
...
searchList.addEventListener("keyup", function () {
  var e,
    t = searchList.value.toLowerCase();
  loadMailData(
    ((e = t),
    allmaillist.filter(function (t) {
      return (
        -1 !== t.name.toLowerCase().indexOf(e.toLowerCase()) ||
        -1 !== t.subject.toLowerCase().indexOf(e.toLowerCase())
      );
    }))
  );
}),
